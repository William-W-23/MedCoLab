...
class EQLv2Loss(nn.Module):
    ...
    def __init__(self,
        ...
        """
        Args:
            num_classes:  类别数量（不包含背景类）
            gamma: 映射函数的gamma参数，控制梯度比例映射的陡峭程度
            mu: 映射函数的mu参数，控制梯度比例映射的中心点
            alpha: 正样本权重的缩放因子
            use_distributed:  是否使用分布式训练（多GPU时需要同步梯度统计）
        """
        super().__init__()
        ...
        # 初始设置较高的比值，使得初始权重接近1
        self.register_buffer('pos_neg', torch.ones(num_classes) * 100)

        # 定义映射函数
        def _map_func(x, gamma, mu):
            return 1.0 / (1.0 + torch.exp(-gamma * (x - mu)))
        self.map_func = partial(_map_func, gamma=self.gamma, mu=self.mu)

    def _get_weight(self, pred_scores:  torch.Tensor) -> tuple:
        """
        计算正负样本的权重

        Args:
            pred_scores: 预测分数 [B*N, C] 或 [B, N, C]
        ...
        """
        # neg_w基于pos_neg比例计算
        neg_w = self. map_func(self. pos_neg)  # [num_classes]
        pos_w = 1.0 + self.alpha * (1.0 - neg_w)
        return pos_w, neg_w

    def _collect_grad(self, pred_scores: torch.Tensor, target: torch.Tensor, weight: torch.Tensor):
        """
        收集梯度统计信息用于动态调整权重

        Args:
            pred_scores: 预测分数 [N, C]
            target: one-hot目标 [N, C]
            weight: 权重 [N, C]
        """
    ...
    def forward(self, pred_scores: torch.Tensor, targets: torch.Tensor,
                avg_factor: int = None) -> torch.Tensor:
        """
        计算EQLv2分类损失

        Args:
            pred_scores:  预测分数 [N, C]，logits形式
            targets:  one-hot目标 [N, C]
            avg_factor: 用于归一化的因子

        Returns:
            loss: 计算得到的损失值
        """
        n_i, n_c = pred_scores. size()

        # 获取权重
        pos_w, neg_w = self._get_weight(pred_scores)

        # 扩展权重到与pred_scores相同的形状
        pos_w = pos_w.view(1, -1).expand(n_i, n_c)
        neg_w = neg_w.view(1, -1).expand(n_i, n_c)

        # 组合权重：正样本用pos_w，负样本用neg_w
        weight = pos_w * targets + neg_w * (1 - targets)
        ...
        if avg_factor is not None and avg_factor > 0:
            # 除以 (样本数 * 类别数) 来归一化
            cls_loss = cls_loss. sum() / (n_i * n_c) * (n_i / avg_factor)
        else:
            cls_loss = cls_loss.mean()

        # 收集梯度统计（仅在训练时）
        if self.training:
            self._collect_grad(pred_scores. detach(), targets.detach(), weight.detach())
...
class DETRLoss(nn.Module):
    ...
    def __init__(
        ...
        if loss_gain is None:
            loss_gain = {"class": 1, "bbox": 5, "giou": 2, "no_object": 0.1, "mask": 1, "dice": 1, "eqlv2": 1.0}
        self.nc = nc
        ...
        self.eqlv2 = EQLv2Loss(
            num_classes=nc,
            gamma=eql_gamma,
            mu=eql_mu,
            alpha=eql_alpha,
            use_distributed=False
        ) if use_eqlv2 else None
    ...
    def _get_loss_eqlv2(
        ...
        """使用EQLv2计算分类损失（作为额外的损失项）

        Args:
            pred_scores (torch.Tensor): 预测的类别分数，形状 (B, N, C)
            targets (torch. Tensor): 目标类别索引，形状 (B, N)
            gt_scores (torch.Tensor): 真实置信度分数，形状 (B, N)
            num_gts (int): 真实目标数量
            postfix (str, optional): 损失名称后缀

        Returns:
            (dict[str, torch. Tensor]): 包含EQLv2分类损失的字典
        """
        name_eqlv2 = f"loss_eqlv2{postfix}"
        bs, nq = pred_scores.shape[: 2]

        # 创建one-hot编码，注意这里不包含背景类
        one_hot = torch.zeros((bs, nq, self.nc), dtype=torch.float32, device=targets.device)

        # 只对非背景类的目标进行one-hot编码
        valid_mask = targets < self.nc
        if valid_mask.any():
            valid_indices = valid_mask.nonzero(as_tuple=True)
            valid_targets = targets[valid_mask]
            one_hot[valid_indices[0], valid_indices[1], valid_targets] = 1.0

        # 将 (B, N, C) 展平为 (B*N, C) 用于EQLv2计算
        pred_flat = pred_scores. view(-1, self.nc)
        target_flat = one_hot.view(-1, self. nc)

        # 使用EQLv2计算损失
        loss_eqlv2 = self. eqlv2(pred_flat, target_flat, avg_factor=max(num_gts, 1))

        return {name_eqlv2: loss_eqlv2.squeeze() * self.loss_gain. get("eqlv2", 1.0)}
    ...
    def _get_loss(
        ...
        # 额外添加EQLv2损失
        if self.use_eqlv2 and self.eqlv2 is not None:
            losses.update(self._get_loss_eqlv2(pred_scores, targets, gt_scores, len(gt_bboxes), postfix))
